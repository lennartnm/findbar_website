...
function RoiBarChart({ data }: { data: RoiDatum[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto">
      <line x1={margin.left} x2={margin.left} y1={margin.top} y2={margin.top + innerH} stroke="#cbd5e1" />
      <line x1={margin.left} x2={margin.left + innerW} y1={zeroY} y2={zeroY} stroke="#94a3b8" />
      {[maxY, 0, minY].map((t, i) => (
        <g key={i} transform={`translate(0, ${yScale(t)})`}>
          <line x1={margin.left} x2={margin.left + innerW} y1="0" y2="0" stroke="#e2e8f0" strokeDasharray="3 4" />
          <text x={margin.left - 8} y={0} fontSize="11" textAnchor="end" dominantBaseline="central" fill="#0f172a">
            {k(t)} €
          </text>
        </g>
      ))}
      {data.map((d, i) => {
        ...
        return (
          <g key={i}>
            ...
            <text x={x + barW / 2} y={zeroY + 16} fontSize="11" textAnchor="middle" fill="#0f172a">
              {d.year}
            </text>
          </g>
        );
